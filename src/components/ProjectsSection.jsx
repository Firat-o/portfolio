        ...
        <div className="grid grid-cols-1 md:grid-cols-6 md:grid-rows-2 gap-4 h-full">
          {projects.map((project, i) => (
            <motion.div
              ...
              className={cn(
                "group relative overflow-hidden tech-card p-0 flex flex-col justify-end min-h-[350px]",
            ...
            >
              ...
              <div className="relative z-20 p-8 space-y-4">
                ...
                <p className="text-muted-foreground text-sm leading-relaxed max-w-md opacity-0 group-hover:opacity-100 transform translate-y-4 group-hover:translate-y-0 transition-all duration-500">
                  {project.description}
                </p>
                ...
                <div className="flex items-center justify-between pt-4 border-t border-white/5">
                  <div className="flex gap-2">
                    {project.tags.map(tag => (
                      <span key={tag} className="text-[9px] font-mono border border-primary/20 px-2 py-0.5 rounded-full bg-primary/5">
                        {tag}
                      </span>
                    ))}
                  </div>
                  <div className="flex gap-4">
                    {project.githubUrl !== "#" && (
                      <a href={project.githubUrl} target="_blank" className="text-muted-foreground hover:text-primary transition-colors">
                        <Github size={20} />
                      </a>
                    )}
                    <a href={project.demoUrl} target="_blank" className="p-2 bg-primary text-primary-foreground rounded-full hover:scale-110 transition-transform">
